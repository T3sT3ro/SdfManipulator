...
public class SourceGeneratingVisitor {
    ...
    public string Visit(VariableDeclaration node) {
        ...
        var typeModifier = node.typeModifier != null ? Stringifier.Get(node.typeModifier.GetValueOrDefault()) : null;
    ...
    public string Visit(StructDeclaration.Member node) {
        var interpolation = node.interpolation != null ? Stringifier.Get(node.interpolation.GetValueOrDefault()) : null;
