        ...
        Pass
        {
            ...
            */
            ...
            float3 __SDF_NORMAL(float3 p)
            {
                ...
                return normalize(
                    k.xyy * sdfScene(p + k.xyy * _EPSILON_NORMAL).distance +
                    k.yyx * sdfScene(p + k.yyx * _EPSILON_NORMAL).distance +
                    k.yxy * sdfScene(p + k.yxy * _EPSILON_NORMAL).distance +
                    k.xxx * sdfScene(p + k.xxx * _EPSILON_NORMAL).distance
                );
            }
            ...
            SdfResult castRay(inout Ray3D ray)
            {
                SdfResult sdf = (SdfResult)0;
                float d = ray.marchedDistance;
            ...
            Ray3D getRaysForCamera(float3 screenPos, float3 objectHitpos)
            {
                ...
                ray.ro = ro;
                ray.rd = rd;
                ray.marchedDistance += _RAY_ORIGIN_BIAS;
